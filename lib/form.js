...
form.addEventListener("submit", async (e) => {
  ...
  hideMessage();
  try {
    const response = await fetch("/send", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lang, name, email, message, turnstileToken }) });
...
function showMessage(text, type) {
  ...
  if (type === "success") setTimeout(hideMessage, 5555);
}
